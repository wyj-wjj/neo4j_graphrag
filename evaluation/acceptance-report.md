...
- 后端 Ruff 与 mypy strict；pytest 47 通过、1 个真实依赖用例按环境条件跳过，分支覆盖率 86.48%。
...
当前工作环境无 Docker CLI，且 Playwright Chromium 下载被网络环境截断，因此以下项目不能诚实标记为本地已通过：

- MySQL 8.4、Redis 8.2、Milvus 2.6、Neo4j 5.26 真实 Adapter 与迁移；
- 后端/前端镜像构建、非 root 运行、健康检查和 Trivy；
- Chromium E2E、键盘流程和 axe 自动可访问性扫描。

对应 GitHub Actions Job 已作为必需门禁实现。只有全部 Job 在目标仓库实际全绿，阶段一外部环境验收才完成。
