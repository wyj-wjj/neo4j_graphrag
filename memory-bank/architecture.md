...
FastAPI 模块化单体
    ...
    └── Port/Adapter
         ...
         ├── Neo4j 5.26 / tenant-scoped bounded traversal
...
- 真实依赖：Compose 启动 MySQL/Redis/Milvus/Neo4j，执行迁移与 Adapter 集成测试。
- 镜像：后端与非 root Nginx 多阶段构建，Trivy 阻断已修复的 Critical 漏洞。
- Secret：gitleaks。
